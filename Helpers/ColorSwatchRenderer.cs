...
// Draws a small colour box before each colour token in AvalonEdit.
// ============================================================
...
namespace QuickLook.Plugin.DevPowerTool
{
    ...
    public sealed class ColorSwatchRenderer : IBackgroundRenderer
    {
        ...
        public void Draw(TextView textView, DrawingContext ctx)
        {
            ...
            // Measure margin width once
            if (_marginWidth < 0)
            {
                try
                {
                    var transform = textView.TransformToAncestor(_editor.TextArea);
                    _marginWidth  = transform.Transform(new Point(0, 0)).X;
                }
                catch { _marginWidth = 40; }
            }
            ...
            foreach (var s in _swatches)
            {
                ...
                var pen = new Pen(border, 0.8);
                pen.Freeze();

                ctx.DrawRoundedRectangle(fill, pen,
